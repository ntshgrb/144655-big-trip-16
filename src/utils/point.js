...
const sortData = (eventsTypesTitles, eventsTypesTotalCosts) => {
  const sortedData = eventsTypesTitles.map( (d, i) => ({label: d, data: eventsTypesTotalCosts[i]}));

  return sortedData.sort((a, b) => b.data - a.data);
};
...
export {sortDateDown, sortDurationDown, sortPriceDown, getDelta, renderDuration, isFutureEvent, isCurrentEvent, sortData, sumTotalCostByType, countEventsByType, sumTotalTimeByType};
